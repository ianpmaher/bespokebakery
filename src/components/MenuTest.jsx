            ...
            <div className="flex-1">
              <ul className="">
                {item.items.map((subItem, subIndex) => (
                  <li key={subIndex} className="border-b border-gray-300 py-2 hover:text-bake-espresso-dark hover:bg-baguette-crust-tan">
                    <h3 className="text-xl font-medium">{subItem.name}</h3>
                    <p>
                      ingredients |<span className="font-bold ml-1">{subItem.ingredients.join(", ")}</span>
                    </p>
                    <p className="mt-1">
